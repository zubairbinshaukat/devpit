...
| 19 | Nothing leaves the machine unless usage stats are on | off by default; `telemetry.Enabled` needs `telemetry_opt_in` and refuses when `DEVPIT_NO_TELEMETRY` or `DO_NOT_TRACK` holds anything but "", `0` or `false`; the clean screen only queues its report command when `Enabled` says so | `TestDefaults` in `config`; `TestEnabledHonoursTheOptInAndBothKillSwitches` in `telemetry`; `TestNothingIsReportedWhenStatsAreOff` in the clean screen |
| 19a | A usage-stats report carries counts and rule names, never a path, a project name or a machine name | `telemetry.Build` reads only `clean.Report`'s counts and a path→rule map, and a type key that is not `[a-z0-9._-]`, is over 40 characters or is one of more than 32 keys is dropped rather than sent; the OS version is sanitized to `[0-9A-Za-z. _-]` and 32 characters | `TestBuildNeverCarriesAPathOrAName`, `TestBuildDropsTypeKeysThatAreNotRuleNames`, `TestBuildCapsTheNumberOfTypes`, `TestOSVersionIsBoundedAndPlain`, `TestSendPostsExactlyTheAgreedBody` in `telemetry`; `TestFinishedCleanupReportsWhenStatsAreOn` in the clean screen |
| 19b | Nothing is reported when a run deleted nothing or was cancelled, and a failed report never blocks, retries, queues to disk or shows an error | `telemetry.Build` returns ok=false for a report with no deletions and no freed bytes; `telemetry.Send` bounds itself with a three second timeout, ignores the response, and the clean screen's command discards its error and returns no message | `TestBuildReportsNothingWhenNothingWasDeleted`, `TestSendSwallowsWhateverTheServerDoes` in `telemetry`; `TestCancelledCleanupSendsNothing` in the clean screen |
